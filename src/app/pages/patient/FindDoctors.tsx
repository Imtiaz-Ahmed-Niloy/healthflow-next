    ...
    <PatientPortalLayout>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="bg-chip relative min-h-[280px]">
          <Image src={patientSarah} alt="Dr. Sarah Liao" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" />
        </div>
      </motion.div>
        ...
        <div className="mt-5 space-y-3">
          ...
          {listVisible.map((d, i) => (
            <motion.div key={d.name} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <Image src={d.img} alt={d.name} width={48} height={48} className="h-12 w-12 rounded-xl object-cover" />
