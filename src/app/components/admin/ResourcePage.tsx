...
import { Upload, X, Plus, Globe, FileText, Paperclip, User, ChevronLeft, ChevronRight } from "lucide-react";
...
const SOCIAL_PLATFORMS: { key: string; label: string; Icon: ComponentType<{ className?: string }> }[] = [
    ...
    <>
      <Card className="p-5">
        <Toolbar
          ...
          onAdd={() => { setStep(0); setCreating(true); }} addLabel={config.addLabel || "New"}
      ...
      {/* Create / Edit */}
      <Modal open={creating || !!editing} onClose={() => { setCreating(false); setEditing(null); setStep(0); }}
        size="lg"
        title={editing ? "Edit record" : "Create new"}
        footer={<>
          <button onClick={() => { setCreating(false); setEditing(null); setStep(0); }} className="px-4 py-2 rounded-full text-sm font-semibold border border-border">Cancel</button>
          {steps && !isFirstStep && (
        ...
        <form id="resource-form" onSubmit={e => {
          ...
          setCreating(false); setEditing(null); setStep(0);
        }}>
          <div className="grid grid-cols-2 gap-x-4">
            {config.fields.map(f => {
