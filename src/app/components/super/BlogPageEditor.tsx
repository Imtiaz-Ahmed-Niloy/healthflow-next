...
import { defaultBlogContent, useBlogContent, type BlogContent, type Post } from "@/data/cmsBlog";

const imageFieldValue = (src: Post["cover"]) => (typeof src === "string" ? src : src.src);
...
const BlogPageEditor = () => {
  ...
  const onReset = () => { reset(); setDraft(defaultBlogContent); setDirty(false); toast.success("Blog page reset"); };
                ...
                <div key={p.slug + i} className="rounded-xl border border-border/60">
                  ...
                  {open && (
                    <div className="px-4 pb-4 space-y-3 border-t border-border/60 pt-4">
                      ...
                      <div className="grid md:grid-cols-2 gap-3">
                        <div className="space-y-1.5"><Label>Cover image URL</Label><Input value={imageFieldValue(p.cover)} onChange={e => setPost(i, { cover: e.target.value })} /></div>
                        <div className="space-y-1.5"><Label>Author photo URL</Label><Input value={imageFieldValue(p.authorPhoto)} onChange={e => setPost(i, { authorPhoto: e.target.value })} /></div>
                      </div>
