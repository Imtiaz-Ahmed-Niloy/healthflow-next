        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {visible.map((d, i) => (
            <motion.article key={d.name}
              ...
              <Link to={`/doctors/${slugify(d.name)}`} className="mt-5 block text-center w-full rounded-full bg-primary py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary-glow transition-colors">Book Appointment</Link>
            </motion.article>
          ))}
        </div>
      )}
      <div className="mt-8 flex justify-end">
        <Link to="/doctors" className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:gap-2 transition-all">View All Doctors <ArrowRight className="h-4 w-4" /></Link>
      </div>
    </section>
  );
};
