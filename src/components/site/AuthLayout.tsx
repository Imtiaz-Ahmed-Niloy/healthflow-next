    ...
    <header className="bg-background border-b border-border/50">
      <nav className="container mx-auto flex items-center justify-between py-4">
        ...
        <ul className="hidden md:flex items-center gap-10 text-xs font-bold tracking-widest">
          {navLinks.map(l => (
            <li key={l.to}>
              <Link to={l.to} className={`transition-colors ${pathname === l.to ? "text-primary-glow" : "text-foreground/70 hover:text-primary"}`}>
                {l.label.toUpperCase()}
              </Link>
            </li>
          ))}
        </ul>
        <Link to="/signup" className="rounded-full bg-gradient-dark text-surface-dark-foreground px-6 py-2.5 text-xs font-bold tracking-wider hover:opacity-90 transition-opacity">GET STARTED</Link>
      </nav>
    </header>
  );
};
  ...
  <footer className="border-t border-border/50 mt-auto bg-background">
    <div className="container mx-auto py-5 flex flex-wrap items-center justify-between gap-4 text-[11px] tracking-widest font-semibold text-muted-foreground">
      ...
      <ul className="flex flex-wrap gap-6">
        ...
        <li><Link to="/data-use" className="hover:text-primary">DATA USE POLICY</Link></li>
        <li><Link to="/cookies" className="hover:text-primary">COOKIE SETTINGS</Link></li>
      </ul>
    </div>
  </footer>
);
