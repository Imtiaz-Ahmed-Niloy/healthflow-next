        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((p, i) => (
            <motion.div key={p.name}
              ...
              <Link href="/pricing" className={`mt-8 block text-center w-full rounded-full py-3 text-sm font-semibold transition-all ${p.featured ? "bg-primary text-primary-foreground hover:bg-primary-glow" : "border border-surface-dark-foreground/30 hover:bg-surface-dark-foreground/10"}`}>
                {p.cta}
              </Link>
            </motion.div>
          ))}
