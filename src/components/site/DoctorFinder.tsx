...
/**
 * Finding a doctor: the search, specialty, gender and location filters, the
 * "N doctors found" line with a chip per active filter, the sort, and the
 * grid of cards. One component for the public /doctors page and the
 * patient's /patient/find-doctors, so the two stay the same.
 *
 * `action` is each card's button — a patient's opens the booking form; left
 * out, the card keeps its own (the doctor's profile). `initial` carries what
 * the homepage search bar sent along.
 */
...
// Trial: white fields, here only. The shared FILTER_CONTROL (grey) is left
        ...
        <div className="flex flex-col gap-2 lg:flex-row">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:flex">
            ...
            <Select value={gender} onValueChange={v => setGender(v as Gender)}>
              <SelectTrigger className={`${CONTROL} lg:w-44`} aria-label={t("genderLabel")}>
