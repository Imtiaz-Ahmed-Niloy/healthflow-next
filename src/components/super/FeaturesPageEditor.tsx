...
const FeaturesPageEditor = () => {
  ...
  const onSave = async () => {
    try {
      await save(draft);
      setDirty(false);
      toast.success(t("features.updated"));
    } catch (cause) {
      toast.error(describeError(cause, t("features.saveFailed")));
    }
  };
  const onReset = async () => {
    try {
      await reset();
      toast.success(t("features.reset"));
    } catch (cause) {
      toast.error(describeError(cause, t("features.resetFailed")));
    }
  };
  ...
  const bar = <SaveBar dirty={dirty} onSave={onSave} onReset={onReset} />;

  return (
    <Tabs defaultValue="hero" className="space-y-4">
      <TabsList className="flex flex-wrap h-auto">
        <TabsTrigger value="hero">{t("hero")}</TabsTrigger>
        <TabsTrigger value="architecture">{t("features.architecture")}</TabsTrigger>
        <TabsTrigger value="logic">{t("features.logic")}</TabsTrigger>
        <TabsTrigger value="core">{t("features.coreTab")}</TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="architecture">
        <Card className="p-5">
          <SectionTitle title={t("features.architecture")} action={bar} />
          <div className="grid md:grid-cols-2 gap-3 mb-4">
            <div className="space-y-1.5"><Label>{t("sectionTitle")}</Label>
              <Input value={arch.title} onChange={e => setArch({ title: e.target.value })} /></div>
            <div className="space-y-1.5"><Label>{t("features.tabs")}</Label>
              <Input value={arch.tabs.join(", ")} onChange={e => setArch({ tabs: e.target.value.split(",").map(s => s.trim()).filter(Boolean) })} /></div>
          </div>
          <div className="space-y-1.5 mb-5"><Label>{t("subtitle")}</Label>
            <Textarea rows={2} value={arch.subtitle} onChange={e => setArch({ subtitle: e.target.value })} /></div>

          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-primary">{t("features.cards", { count: arch.items.length })}</h3>
            <Btn variant="outline" onClick={addArchItem}><span className="inline-flex items-center gap-1"><Plus className="h-4 w-4" />{t("features.addCard")}</span></Btn>
          </div>
            ...
            {arch.items.map((it, i) => (
              <div key={i} className="rounded-xl border border-border/60 p-4 space-y-2">
                <div className="grid md:grid-cols-[160px_1fr_auto] gap-2 items-end">
                  <div className="space-y-1.5"><Label>{t("icon")}</Label><IconSelect value={it.icon} onChange={v => setArchItem(i, { icon: v })} /></div>
                  <div className="space-y-1.5"><Label>{t("title")}</Label><Input value={it.title} onChange={e => setArchItem(i, { title: e.target.value })} /></div>
                ...
                <div className="space-y-1.5"><Label>{t("features.bullets")}</Label>
      ...
      <TabsContent value="logic">
        <Card className="p-5">
          <SectionTitle title={t("features.logic")} action={bar} />
          <div className="grid md:grid-cols-2 gap-3 mb-3">
            <div className="space-y-1.5"><Label>{t("title")}</Label>
              <Input value={logic.title} onChange={e => setLogic({ title: e.target.value })} /></div>
            <div className="space-y-1.5"><Label>{t("features.accentTitle")}</Label>
              <Input value={logic.accentTitle} onChange={e => setLogic({ accentTitle: e.target.value })} /></div>
          </div>
          <div className="space-y-1.5 mb-3"><Label>{t("description")}</Label>
            <Textarea rows={3} value={logic.description} onChange={e => setLogic({ description: e.target.value })} /></div>
          <div className="space-y-1.5 mb-5"><Label>{t("features.ctaLabel")}</Label>
            <Input value={logic.ctaLabel} onChange={e => setLogic({ ctaLabel: e.target.value })} /></div>

          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-primary">{t("features.points", { count: logic.points.length })}</h3>
            <Btn variant="outline" onClick={addLogicPt}><span className="inline-flex items-center gap-1"><Plus className="h-4 w-4" />{t("features.addPoint")}</span></Btn>
          </div>
          <div className="space-y-3">
            {logic.points.map((p, i) => (
              <div key={i} className="rounded-xl border border-border/60 p-4 space-y-2">
                <div className="grid md:grid-cols-[160px_1fr_auto] gap-2 items-end">
                  <div className="space-y-1.5"><Label>{t("icon")}</Label><IconSelect value={p.icon} onChange={v => setLogicPt(i, { icon: v })} /></div>
                  <div className="space-y-1.5"><Label>{t("title")}</Label><Input value={p.title} onChange={e => setLogicPt(i, { title: e.target.value })} /></div>
      ...
      <TabsContent value="core">
        <Card className="p-5">
          <SectionTitle title={t("features.core")} action={bar} />
          <div className="grid md:grid-cols-2 gap-3 mb-5">
            <div className="space-y-1.5"><Label>{t("title")}</Label>
              <Input value={core.title} onChange={e => setCore({ title: e.target.value })} /></div>
            <div className="space-y-1.5"><Label>{t("subtitle")}</Label>
              <Input value={core.subtitle} onChange={e => setCore({ subtitle: e.target.value })} /></div>
          </div>

          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-primary">{t("features.coreCards", { count: core.items.length })}</h3>
            <Btn variant="outline" onClick={addCoreItem}><span className="inline-flex items-center gap-1"><Plus className="h-4 w-4" />{t("features.addCard")}</span></Btn>
          </div>
          <div className="space-y-3">
            {core.items.map((it, i) => (
              <div key={i} className="rounded-xl border border-border/60 p-4 space-y-2">
                <div className="grid md:grid-cols-[160px_1fr_auto] gap-2 items-end">
                  <div className="space-y-1.5"><Label>{t("icon")}</Label><IconSelect value={it.icon} onChange={v => setCoreItem(i, { icon: v })} /></div>
                  <div className="space-y-1.5"><Label>{t("title")}</Label><Input value={it.title} onChange={e => setCoreItem(i, { title: e.target.value })} /></div>
                ...
                <div className="space-y-1.5"><Label>{t("features.chips")}</Label>
