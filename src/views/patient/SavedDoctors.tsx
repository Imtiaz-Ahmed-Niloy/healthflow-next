  ...
  return (
    <PatientPortalLayout>
      <div className="max-w-xl">
        <h1 className="font-display text-5xl text-primary">Saved Doctors</h1>
        <p className="text-sm text-muted-foreground mt-3">
          Doctors you saved from their profile. Book one from here, or tap the heart to remove them.
        </p>
      </div>
      ...
      <div className="mt-8">
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border/60 p-10 text-center">
            ...
            <p className="font-display text-2xl text-primary mt-3">No saved doctors yet</p>
            <p className="text-sm text-muted-foreground mt-1">
              Press Save on a doctor&apos;s profile and they&apos;ll be here.
            </p>
                ...
                action={
                  <div className="mt-5 flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => void toggle(d)} disabled={busy}
                      title={`Remove ${d.name} from saved doctors`} aria-label={`Remove ${d.name} from saved doctors`}
